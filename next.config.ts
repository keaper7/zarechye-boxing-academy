...
const nextConfig: NextConfig = {
  ...
  output: 'export',
  basePath,
  // Тот же префикс, но доступный из кода компонентов. Нужен там, где
  // путь к файлу в public/ пишется руками: next/image и <Link> получают
  // basePath автоматически, а обычный <img src="/gym/...">, из которого
  // собран srcset галереи, — нет, и в продакшне ловил бы 404.
  // Значение объявлено здесь одно на весь проект, чтобы имя репозитория
  // не пришлось дублировать во втором файле и однажды забыть поправить.
