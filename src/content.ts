...
// Адрес и телефон зала — от заказчика напрямую (Айдамир).
// Номер текстом на сайте не показывается (решение заказчика) — но
// живёт здесь константой, потому что нужен для ссылки WhatsApp.
...
export const content = {
  ...
  contact: {
    ...
    /** VERIFIED: оба Instagram-аккаунта — из профилей. WhatsApp ведёт на тот
     *  же номер, что раньше был показан текстом на сайте (contact.place),
     *  но сам номер цифрами на странице больше не выводится нигде —
     *  решение заказчика. У WhatsApp-пункта поэтому нет `handle`: остальные
     *  пункты подписаны текстом-адресом (@ник), а этот — просто ссылка. */
    links: [
      ...
      { label: 'WhatsApp', href: WHATSAPP_COACH },
    ],
    /** Прямые адреса для кнопок вне списка контактов — см. комментарий
     *  у констант вверху файла. */
    directHref: IG_GYM,
    coachHref: IG_COACH,
    /** Адрес — от заказчика напрямую. Телефон текстом на сайте больше не
     *  показываем — заказчик попросил убрать номер со страницы полностью;
     *  связь теперь только через кнопки (директ, WhatsApp), а не звонок
     *  по видимому номеру. Сам номер (WHATSAPP_COACH) по-прежнему нужен —
     *  он просто зашит в ссылку WhatsApp, а не напечатан рядом с ней. */
    place: {
      enabled: true,
      area: 'Заречье',
